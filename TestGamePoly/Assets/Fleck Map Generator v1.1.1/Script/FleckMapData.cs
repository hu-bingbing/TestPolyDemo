namespace MoenenGames.FleckMapGenerator {
	...
	public class FleckMap {
        ...
        public List<Vector3> AllGroundPointList = new List<Vector3>();
        ...
        public GameObject SpawnToScene(GameObject prefabWall, GameObject prefabGround, GameObject prefabBirthPoint = null)
        {
            AllGroundPointList.Clear();
            ...
            for (int i = 0; i < Width; i++)
            {
                for (int j = 0; j < Height; j++)
                {
                    ...
                    for (int k = 0; k < alt; k++)
                    {
                        ...
                        if (k == alt - 1 && data[j * Width + i] == 1)
                        {
                            AllGroundPointList.Add(new Vector3(i * tileSize, k * tileSize + 0.5f * tileSize, j * tileSize));
                        }
